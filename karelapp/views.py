...
# IMPORT - MODELY:
from adminapp.models import aktualniden, food_drink, vybava, aktivita_kcal2, kondice_hubnuti, spanek, hraci_v_tymu, pamet_karel, motivacni_citat
from .models import karel_zakladni_staty, karel_snedl, karel_snedl_polozka, karel_equip, InventarPolozkaFood, InventarPolozkaVybava, karel_inv
...
pamet_vcera = pamet_karel.objects.get(den=vcerejsi_den)
...
karlova_aktualni_vaha_vcera = pamet_vcera.karlova_aktualni_vaha

# BMI
karel_BMI = (((zakladni_staty.vaha)/((zakladni_staty.vyska)/100)**2))
karel_BMI = round(karel_BMI,2)

# BMR - 10*VÁHA, + 6,25*VÝŠKA - 5*VĚK +5
karel_BMR = (((zakladni_staty.vaha)*10) + ((zakladni_staty.vyska)*6.25) - ((zakladni_staty.vek)*5) +5)
karel_BMR = (karel_BMR)+((karel_BMR)*(BONUS_BMR_procenta)) #<--- Jelikož v závorce je - tak před ní musí být +
karel_BMR = round(karel_BMR,2)
...
agregovane_hodnoty = vysledky_hracu.objects.filter(tym="karel").aggregate(
...
hraci = vysledky_hracu.objects.filter(tym="karel")
for hrac in hraci:
    hrac.celkem_kroku = (
        hrac.celkem_kroku
    )

# KOLIK KM ZA DEN KAREL UŠEL <--- Bez bonusů, je potřeba na výpočty
delka_kroku = (zakladni_staty.delkakroku)*(BONUS_delka_kroku_procenta)
karel_km_dnes = round((((dnesni_prumer_kroku)*(delka_kroku)/100)/1000), 2)

# KAPACITA

# karluv aktualni batoh
if BONUS_kapacita_flet == 0:
    BONUS_kapacita_flet = 5
kapacita_batohu = (BONUS_kapacita_flet)*(BONUS_kapacita_procenta)

# objem inventáře (všechno co karel má)
karel_inv_instance = karel_inv.objects.get(den=aktualni_den)  # Získání objektu s daným dnem
objem_inv = round(karel_inv_instance.objem_inventare,2)  # Získání hodnoty equip_objem

# objem toho co má karel na sobě (protože to nezabírá místo v batohu)
karel_equip_objem = karel_equip.objects.get(den=aktualni_den)
karel_equip_objem = karel_equip_objem.equip_objem

# karlova volná kapacita
...
tempo = round(((karel_km_dnes)/(aktivita_dnes.karel_aktivita)),1)
if tempo == 0:
    tempo = 3
...
# KALORIE <--- příjem
sum_kcal_dnes = karel_snedl.objects.get(den=aktualni_den)
sum_kcal_dnes = sum_kcal_dnes.snedeno_kcal

# HYDRATACE
hydratace_jidlo = 1
dnesni_zaznam_hydratace = karel_snedl.objects.get(den=aktualni_den)
...
#SYTOST
sytost_jidlo = 0
dnesni_zaznam_sytost = karel_snedl.objects.get(den=aktualni_den)
...
sum_snezene_hmotnost_dnes = karel_snedl.objects.get(den=aktualni_den)
...
doba_aktivity_dnes = doba_aktivity.karel_aktivita # <-- Zjištěno, kolik hodin byl karel aktivní
kcal_vydej_dnes = ((kcal_vydej.kcal_za_hodinu)*1.5)*(doba_aktivity_dnes) # <-- kolik kalorií karel dneska spálil aktivitou 

kcal_vydej_dnes_SUMA = round(kcal_vydej_dnes)+(karel_BMR) # <-- CELKOVÉ SPLÁLENÉ KALORIE VČETNĚ BAZÁLU
...
karel_celkova_vaha = round((karlova_aktualni_vaha_vcera) + (kolik_zhubnul_dnes),2)
...
# ZÁTĚŽ
nosnost = (karel_celkova_vaha) / 5
karel_inv_instance = karel_inv.objects.get(den=aktualni_den)
zatez = round(((karel_inv_instance.vaha_inventare)+(karel_inv_instance.vaha_inventare*BONUS_zatez_procenta)),2)
...
karel_spanek = ((spanek_vcera.karel_spanek)+(BONUS_spanek_flat))*BONUS_spanek_procenta

if karel_spanek ==0:
    karel_spanek=8

karel_aktivita = spanek_vcera.karel_aktivita

if karel_aktivita == 0:
    karel_aktivita=6

# ENERGIE
energie_bonus_jidlo = karel_snedl.objects.get(den=aktualni_den)
energie_jidlo = energie_bonus_jidlo.snedeno_energie

karel_energie = ((energie_vcera)+(energie_jidlo)+(karel_spanek*20)+(kcal_prijem_dnes/500)-(200)-(dnesni_prumer_kroku/1000))*((negace_energie_hydratace+negace_energie_sytost)/2)

karel_energie_procenta = round(karel_energie)

if karel_energie<0.1:
    karel_energie_procenta=0.1
if karel_energie>150:
    karel_energie_procenta=150

if karel_energie_procenta <30:
    negace_kondice_energie = 0.95
elif karel_energie_procenta <15:
...
hodnota = karel_energie_procenta
...
vykon_procenta = round((((karel_energie_procenta)*(koeficient_energie))+(negace_hubnuti_procenta)+((kondice_procenta)*(koeficient_kondice))+((koeficient_hydratace)*(hydratace_procenta))+((sytost_procenta)*(koeficient_sytosti))+((zatez_procenta)*(koeficient_zateze))+((volna_kapacita_procenta)*(koeficient_volne_kapacity)))/7) #<-- 6=počet atributů (děláme průměr)
...
karel_xp = round(((((suma_kroky/100)/(pocet_hracu_karel))+(XP_bonus_flat))*BONUS_XP_procenta))
...
for lvl in levely.values():
    if karel_xp > lvl:
        ...
        next_lvl_jeste_chybi = xp_na_next_lvl-karel_xp
        xp_next_lvl = ((karel_xp)+(xp_na_next_lvl))-karel_xp
    else:
...
# HODNOTA INV
inv_cena = karel_inv.objects.get(den=aktualni_den)
inv_suma_cena = inv_cena.cena_inv


karel_final_km_vcera = pamet_karel.objects.get(den=vcerejsi_den)
final_km_vcera = karel_final_km_vcera.km_s_bonusem
KAREL_FINAL_KM = round((final_km_vcera+(((karel_km_dnes*vykon_konstanta))*(bonus_km_lvl))),2)

karel_km_bonus_dnes = round(((karel_km_dnes*vykon_konstanta)*bonus_km_lvl),2)
...
# PŘEJMENOVÁNÍ:
karel_kondice_procenta = kondice_procenta
karel_sytost_procenta = sytost_procenta
karel_hydratace_procenta = hydratace_procenta
karel_zatez_procenta = zatez_procenta
karel_volna_kapacita_procenta = volna_kapacita_procenta

def karel_main_page(request):

    context = {}
    context['karel_xp'] = karel_xp
...
# TO NEJDŮLEŽITĚJŠÍ:
    context['KAREL_FINAL_KROKY'] = KAREL_FINAL_KM

# HLAVNÍ STATY:
    context['vykon_procenta'] = vykon_procenta

    context['volna_kapacita_procenta'] = karel_volna_kapacita_procenta
    context['zatez_procenta'] = karel_zatez_procenta
    context['kondice_procenta'] = karel_kondice_procenta
    context['karel_energie_procenta'] = karel_energie_procenta
    context['hydratace_procenta'] = karel_hydratace_procenta
    context['sytost_procenta'] = karel_sytost_procenta

    context['aktualni_den'] = aktualni_den
    context['denni_prumer_kroku'] = round(dnesni_prumer_kroku)
    context['karel_km_dnes'] = karel_km_dnes
...
# POMOCNÉ STATY:
    ...
    context['karel_spanek'] = karel_spanek
    context['karel_aktivita'] = karel_aktivita
    context['zatez'] = zatez
    context['karel_bmr'] = karel_BMR
    context['karel_zakladni_vaha'] = zakladni_staty.vaha
    context['karel_celkova_vaha'] = karel_celkova_vaha
    context['karel_vyska'] = zakladni_staty.vyska
    context['karel_vek'] = zakladni_staty.vek
    context['karel_pohlavi'] = zakladni_staty.pohlavi



    return render(request, 'karelapp/karel_main_page.html', context)


def full_tym(request):
    hraci = vysledky_hracu.objects.filter(tym="karel")
    return render(request, 'karelapp/karel_full_tym.html', {
        'karel_vsichni_hraci': hraci
    })
...
def detail_hrace(request, id):
    tym_karel = vysledky_hracu.objects.filter(tym="karel")
...
# ZJIŠTĚNÍ TÝMOVÉHO POŘADÍ HRÁČE
    try:
        IDporadi_hrace = vysledky_hracu.objects.get(id=id)
        tym_karel = vysledky_hracu.objects.filter(tym="karel").order_by('celkem_kroku')
        poradi_v_tymu = 0
        for hrac in tym_karel:
            if IDporadi_hrace.celkem_kroku <= hrac.celkem_kroku:
                poradi_v_tymu += 1
            else:
                pass
...
karel_pamet = pamet_karel.objects.all()
...
def detail_statistika(request):
    ...
    context['aktivita'] = aktivita_dnes.karel_aktivita
    context['karel_km_dnes'] = karel_km_dnes
    context['karel_km_bez_bonusu'] = karel_km_bonus_dnes
    context['karel_celkova_vaha'] = karel_celkova_vaha
    context['XP_DNES'] = XP_DNES
    context['KAREL_FINAL_KM'] = KAREL_FINAL_KM
    ...
    return render(request, 'karelapp/karel_detail_statistika.html', context)




def jednotlive_dny(request):
    return render(request, 'karelapp/karel_jednotlive_dny.html', {
        'karel_pamet': karel_pamet,
    })