...
class brunhilda_zakladni_staty(models.Model):
...
class brunhilda_snedl(models.Model):
    ...
    def aktualizuj_souhrnne_hodnoty(self):
        ...
        celkem_hmotnost = 0
        celkem_energie = 0
        # 'polozky' je related_name z modelu brunhilda_snedl_polozka
...
class brunhilda_snedl_polozka(models.Model):
    den = models.IntegerField(default=1, blank=True)
    brunhilda_snedl = models.ForeignKey(brunhilda_snedl, on_delete=models.CASCADE, related_name='polozky')
    # OPRAVA: Přidán related_name, aby se předešlo budoucím kolizím
    jidlo = models.ForeignKey('adminapp.food_drink', on_delete=models.CASCADE, related_name='brunhilda_snedl_polozka')
    ...
    def save(self, *args, **kwargs):
        ...
        self.den = self.brunhilda_snedl.den
        super().save(*args, **kwargs)
        # Po uložení položky aktualizujeme souhrnné hodnoty v brunhilda_snedl
        self.brunhilda_snedl.aktualizuj_souhrnne_hodnoty()

    def __str__(self):
        return f'{self.mnozstvi}x {self.jidlo.food_name}'

class brunhilda_equip(models.Model):
    den = models.IntegerField(validators=[MinValueValidator(0)], null=True)
    # OPRAVA: Všechny related_name jsou nyní unikátní pro 'brunhildaapp'
    equip_boty = models.ForeignKey('adminapp.vybava', on_delete=models.CASCADE, related_name='brunhilda_equip_boty', null=True, blank=True)
    equip_ponozky = models.ForeignKey('adminapp.vybava', on_delete=models.CASCADE, related_name='brunhilda_equip_ponozky', null=True, blank=True)
    equip_kalhoty = models.ForeignKey('adminapp.vybava', on_delete=models.CASCADE, related_name='brunhilda_equip_kalhoty', null=True, blank=True)
    equip_triko = models.ForeignKey('adminapp.vybava', on_delete=models.CASCADE, related_name='brunhilda_equip_triko', null=True, blank=True)
    equip_doplnek = models.ForeignKey('adminapp.vybava', on_delete=models.CASCADE, related_name='brunhilda_equip_doplnek', null=True, blank=True)
    equip_batoh = models.ForeignKey('adminapp.vybava', on_delete=models.CASCADE, related_name='brunhilda_equip_batoh', null=True, blank=True)
    equip_spacak = models.ForeignKey('adminapp.vybava', on_delete=models.CASCADE, related_name='brunhilda_equip_spacak', null=True, blank=True)
    equip_karimatka = models.ForeignKey('adminapp.vybava', on_delete=models.CASCADE, related_name='brunhilda_equip_karimatka', null=True, blank=True)
...
class InventarPolozkaFood(models.Model):
    inventar = models.ForeignKey('brunhilda_inv', on_delete=models.CASCADE, related_name='potraviny')
    # OPRAVA: Přidán unikátní related_name
    polozka = models.ForeignKey('adminapp.food_drink', on_delete=models.CASCADE, related_name='brunhilda_inventar_food')
    mnozstvi = models.IntegerField(default=1, validators=[MinValueValidator(0)])
...
class InventarPolozkaVybava(models.Model):
    inventar = models.ForeignKey('brunhilda_inv', on_delete=models.CASCADE, related_name='vybaveni')
    # OPRAVA: Přidán unikátní related_name
    polozka = models.ForeignKey('adminapp.vybava', on_delete=models.CASCADE, related_name='brunhilda_inventar_vybava')
    mnozstvi = models.IntegerField(default=1, validators=[MinValueValidator(0)])

    def __str__(self):
        return f'{self.mnozstvi}x {self.polozka.item_name} v inventáři pro den {self.inventar.den}'

class brunhilda_inv(models.Model):
